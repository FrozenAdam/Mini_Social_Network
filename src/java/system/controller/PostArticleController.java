...
public class PostArticleController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String postBy = request.getParameter("txtPostBy");
            int status = 1;
            int id = dao.getTotalArticle() + 1;
            String search = request.getParameter("txtSearch");
            ArticleDTO dto = new ArticleDTO(id, tittle, description, image, postBy, status);
            if (dao.postNewArticle(dto)) {
                url = SUCCESS;
            }
        } catch (Exception e) {
            LOGGER.error("Error at PostArticleController: " + e.getMessage());
        } finally {
